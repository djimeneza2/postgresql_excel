...
class copyfirstyear():
    ...
    def copy_first_in_other_years_improve(self):
        ...
        self.df_inflow_montecarlo=pd.read_csv(f'{self.csv_ruta}{self.csv_df_full}')

        for other_year in np.arange(self.first_year+1,self.last_year+1):
            ...
            if self.identificar_bisiesto(self.first_year)==0 and self.identificar_bisiesto(other_year)==1:
                ...
                df_other_12=pd.DataFrame(np.array(self.df_inflow_montecarlo.loc[indexes_first_12,columns_p]),
                                                        index=indexes_other_12,
                                                        columns=columns_p)
                
                df_other_34=pd.DataFrame(np.array(self.df_inflow_montecarlo.loc[indexes_first_34,columns_p]),
                                                        index=indexes_other_34,
                                                        columns=columns_p)

                
                df_other_56=pd.DataFrame(np.array(self.df_inflow_montecarlo.loc[indexes_first_56,columns_p]),
                                                        index=indexes_other_56,
                                                        columns=columns_p)
                
                df_update=pd.concat([df_other_12,df_other_34,df_other_56])
                
                self.df_inflow_montecarlo.update(df_update)
                ...
                self.df_inflow_montecarlo.update(df_other_12)
                ...
                df_update=pd.concat([df_other_12,df_other_34])
