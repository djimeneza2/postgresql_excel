        ...
        for column_test in self.df_historical.columns:
            ...
            if self.df_historical[column_test].sum()!=0:
                ...
                if column_test in ['YEAR','MONTH','DAY','PERIOD']:
                    ...
                    for i in np.arange(np.shape(self.df_historical)[0]):

                        if self.df_historical.loc[i,'YEAR'] in np.array(self.df_scenarios['YEAR']):

                            self.df_historical.loc[i,'scen']=np.array(self.df_scenarios['SCENARIO'][self.df_scenarios['YEAR']==self.df_historical.loc[i,'YEAR']])[0]

                            self.df_historical.loc[i,'year_scen']=self.first_year

                    self.df_historical=self.df_historical[self.df_historical['scen']>0]

                    self.df_historical=pd.DataFrame(np.array(self.df_historical),columns=self.df_historical.columns)
                    ...
                    for k in self.df_scenarios['SCENARIO']:#[df_scenarios['SCENARIO']<=28]
                        ...
                        self.df_historical_scen=pd.DataFrame(np.array(self.df_historical_scen),columns=self.df_historical_scen.columns)

                        indice_j=0

                        for i in range(np.shape(self.df_historical_scen)[0]):
                            ...
                            date_historical=datetime.datetime(year=int(self.df_historical_scen['year_scen'][i]),
                                                                month=int(self.df_historical_scen['MONTH'][i]),
                                                                day=day_h,
                                                                hour=int(np.array(self.periods_to_hours['HOUR'][self.periods_to_hours['PERIOD']==self.df_historical_scen.loc[i,'PERIOD']])[0]),
                                                                minute=int(np.array(self.periods_to_hours['MINUTE'][self.periods_to_hours['PERIOD']==self.df_historical_scen.loc[i,'PERIOD']])[0]))


                            for j in range(indice_j,np.shape(self.df_inflow_montecarlo)[0]):

                                date_inflows=datetime.datetime(year=int(self.df_inflow_montecarlo['YEAR'][j]),
                                                                month=int(self.df_inflow_montecarlo['MONTH'][j]),
                                                                day=int(self.df_inflow_montecarlo['DAY'][j]),
                                                                hour=int(np.array(self.periods_to_hours['HOUR'][self.periods_to_hours['PERIOD']==self.df_inflow_montecarlo.loc[i,'PERIOD']])[0]),
                                                                minute=int(np.array(self.periods_to_hours['MINUTE'][self.periods_to_hours['PERIOD']==self.df_inflow_montecarlo.loc[i,'PERIOD']])[0]))
                        ...
                        date_inflows_last=datetime.datetime(year=int(self.df_inflow_montecarlo['YEAR'][indice_last_j]),
                                                            month=int(self.df_inflow_montecarlo['MONTH'][indice_last_j]),
                                                            day=int(self.df_inflow_montecarlo['DAY'][indice_last_j]),
                                                            hour=int(np.array(self.periods_to_hours['HOUR'][self.periods_to_hours['PERIOD']==self.df_inflow_montecarlo.loc[indice_last_j,'PERIOD']])[0]),
                                                            minute=int(np.array(self.periods_to_hours['MINUTE'][self.periods_to_hours['PERIOD']==self.df_inflow_montecarlo.loc[indice_last_j,'PERIOD']])[0]))

                        
                        while date_inflows_last<=datetime.datetime(self.first_year,12,31,23,30):
