...
class ModifyDataFrameMultiYears(CreateDataframeOfMultiYears):
    ...
    def funChangeDataInDataframeColumn(self):
        ...
        columnas_unidades=[]
        for m in dataframe_fin.columns:
            if m in ['YEAR','MONTH','DAY','PERIOD']:
                pass
            else:
                columnas_unidades.append(m)
        ...
        dataframe_fin[dataframe_fin[columnas_unidades]>100]=100            
        
        self.data_frame_fin_mod=dataframe_fin

        return self.data_frame_fin_mod

    def funChangeDataInDataframeColumn_improve(self):
    ...
    def funChangeDataInDataframeColumn_modificar_archivo(self,archivo_mantenimiento):

        iterate=np.shape(self.funCalcDifDates())[0] #2850
        range_iterate=np.arange(0,iterate) #0-2849

        dataframe_fin=pd.read_csv(archivo_mantenimiento)
